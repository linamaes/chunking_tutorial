...
```{julia}
   ...
   using DelimitedFiles # For handling delimited files like csv
    ...
    # set files location
    pathin = readdlm("mypathfolder.txt")

    # load files metadata
    # Where are we going to host the data?
    xmap0 = NetCDF.open(string(pathin[1],"t2m_map.nc"))

    # load files metadata
    xbox0 = NetCDF.open(string(pathin[1],"t2m_blocks.nc"));

    # use the 'view' command to read the data lazily
...
Now, let's observe a 10 years from ? to ? time series at the crossing between the equator and the prime meridian.
```{julia}
...
Now, we will access two different chunk files from the same *air_temperature_2m* data set and explore their properties
...
As we notice, the spatial chunk size is 1x720x1440 which is the same than a map layer. 
On the other hand, the box chunk is stored in small blocks with the following dimmensions 90x90x256^[The spatial chunk's size is based on the spatial grid's size. In this example the grid is 720 x 1440 pixels (a map layer). 
Conversely, The box chunk's size is more flexible and set considering the target analyses.].
...
Now, we want to compute the mean and median values for different chunks and across differente axes.
...
## Mean
Our input data is stored on the disk and accessed as DiskArrays. The `DiskArrays.jl` package uses the internal chunking structure of larger than memory arrays and provides special implementations for the `mapreduce` function used in the implementation of the mean for `AbstractArray`.
The following two aggregations access each chunk only once:
...
Additionally, this computation is properly handled it by DiskArrays.jl
:::
...
# but keep in mind, it is very unefficient looping through the dataset. 
