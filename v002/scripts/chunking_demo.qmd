...
* `t2_blocks.nc`: This chunked file aims an intermediate access to both the spatial and temporal dimensions (box chunking)
...
As we notice, the spatial chunk size is 1440x720x1 which corresponds to the dimmensions of one map layer. 
...
## Mean
Our input data is stored on the disk and accessed as DiskArrays. The `DiskArrays.jl` package uses the internal chunking structure that are larger than the memory arrays and provides special implementations for the `mapreduce` function used in the implementation of the mean for `AbstractArray`.
...
* The mean computation is properly handled it by DiskArrays.jl.
:::
...
# |fig-cap: "Multianual median of air temperature at 2 m from 1979 to 2021 pixel-wise."
geoplotsfx(out3[:,:])
```
